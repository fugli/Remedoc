...
# 	lis.append(a.get('href'))
	if flag2 == 1:
		...
		for link1 in lis[1:]:
			...
			soup3=fn(link1)
			div =  soup3.find_all("div", {"id":"listingtabcontent"})
			...
			for item1 in div :
				...
				for a in li:
					...
					for link in a.find_all("a", {"class":"YPTRACK GAQ_C_BUSL"}):
						...
						href1 = link.get('href')
						# soup2 = fn("http://yellowpages.sulekha.com"+ "/calicut/dr-joy-a-v-west-hill-calicut_contact-address")#href1 )
						
						if href1 == "/calicut/dr-joy-a-v-west-hill-calicut_contact-address":
							flag = 1
							print "&&&&&&&&&&&&&&@@@@@@@@@@@@@@@@@@@@"
						...
						if flag ==1:
							...
							sql = "INSERT INTO `Doc Info`(`Phone`, `Address`, `Email`, `Website`, `Contact Person`, `Working hrs`, `Categories`,`About`, `Clinic name`, `Link`, `Link sec`, `City`,`Badge`, `Photoslink`, `Reviews`, `Featuredin`, `RatingTot`) VALUES (" +'"' +lis2["Phone/Mobile"]+'"'+',"'+lis2["Address"]+'"'+',"'+lis2["Email"]+'"'+',"'+lis2["Website"]+'"'+',"'+lis2["Contact Person"]+'"'+',"'+lis2["Working Hours"]+'"'+',"'+lis2["Categories"]+'"'+',"'+lis2["description"]+'"'+',"'+lis2["pull-left"]+'"'+',"'+lis2["Link"]+'"'+',"'+lis2["Link sec"]+'"'+',"'+city+'"'+',"'+lisdic["badge"]+'"'+',"'+str(photolink)+'"'+',"'+str(strList)+'"'+',"'+str(lisfeatr)+'"'+',"'+lisdic["ratingtot"]+'"'+ ")"
