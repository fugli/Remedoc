		...
		for a in li:
			...
			for link in a.find_all("a", {"class":"YPTRACK GAQ_C_BUSL"}):
				...
				try:
					...
					lis2.update({"pull-left" : "errrrrrrr2 Not found"})
					print "errrrrrrr2"
				...
				for item in div2:
					 ...
					 for item2 in li2:
					 	...
					 	for a in dv1:
					 		...
					 		try:
					 			# lis2.update({a.strong.text: a.div.text })
					 			lis2.update({a.strong.text.replace("'","\\'").encode('utf8') : " ".join(a.div.text.split()).replace("'","\\'").encode('utf8') })
					 			# lis2.update({a.strong.text:" ".join(a.div.text.split())})
					 		except:
					 			print "err in line 85"

					 		# f.write( str(a.strong.text)+" ::: " +str(a.div.text))
				f.write ("########@@@@@@#####")
				sql = "INSERT INTO `test`(`Phone`, `Address`, `Email`, `Website`, `Contact Person`, `Working hrs`, `Categories`,`About`, `Clinic name`) VALUES (" +"'" +lis["Phone/Mobile"]+"'"+",'"+lis["Address"]+"'"+",'"+lis["Email"]+"'"+",'"+lis["Website"]+"'"+",'"+lis["Contact Person"]+"'"+",'"+lis["Working Hours"]+"'"+",'"+lis["Categories"]+"'"+",'"+lis["description"]+"'"+",'"+lis["pull-left"]+"'"+")"
