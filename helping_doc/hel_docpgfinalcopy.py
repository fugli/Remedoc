...
for item_city in (city_list):
	item_city = item_city.rstrip()
	if  item_city == "Gurgaon":             ####to be changed
		flag_city= 1
		print "found city"
	elif flag_city == 0:
		continue
	...
	#########to be edited
	# with open('Delhi2 doc_dict_json.json', 'w') as f:    #####dumping dictionary
	...
	for item_locality in locality_list[1:]:
		...
		if  item_locality == "golf_course_road":               ####to be changed
			flag_locality = 1
		...
		for spec in spec_file:
			...
			if spec == "diabetes_care_specialist":   ####to be changed
				flag_spec = 1
				print "found specs"
			elif flag_spec == 0:
				continue
			for num in range (1,1000):
				...
				for link in atag:
					...
					sqlupdate = "SELECT `specs_script` FROM `hel_doc_info2` WHERE `Doc_link`="+'"'+doc_link+'"'
					# print sqlupdate
