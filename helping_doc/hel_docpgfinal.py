...
for item_city in reversed(city_list):
	...
	if  item_city == "Kolkata":             ####to be changed
		flag_city= 1
		print "found city"
	elif flag_city == 0:
		continue
	...
	# 	json.dump(doc_dict_json, f)								####comment above two linis if in same city

	item_city = "_".join(item_city.lower().split())

	for item_locality in locality_list[1:]:
		item_locality = item_locality.rstrip()
		item_locality = "_".join(item_locality.lower().split())
		
		if  item_locality == "diamond_harbour_road":               ####to be changed
			flag_locality = 1
			print "found locality"
		elif flag_locality == 0:
			continue

		
		for spec in spec_file:
			spec = "_".join(spec.lower().split())
			if spec == "trichologist":   ####to be changed
				flag_spec = 1
				print "found specs"
			elif flag_spec == 0:
				continue
			for num in range (1,1000):
				url = 'https://www.helpingdoc.com/'+spec+'-in-'+item_locality+'-'+item_city+'?page='+str(num)
				# url = "https://www.helpingdoc.com/bleeding_gums_specialist-in-east_delhi"
				# print url
				# print >>open ("terminal.txt",'a'),url
				while True:
					try:
						soup2 = fn(url)
						print 'url::',url
				...
				try:
					if num == 1:
						num_tfoot = soup2.find("tfoot")
						num_atag = num_tfoot.find_all("a")
						total_pgs = len(num_atag)
						if total_pgs == 2:
							total_pgs = total_pgs
						else:
				...
				if num==2 and flag_pgs ==1:
					print "one paage broken"
					flag_pgs = 0
					break
				...
				for link in atag:
					doc_link = 'https://www.helpingdoc.com'+str(link['href'])
					print doc_link
					# print >>open ("terminal.txt",'a'),doc_link

					####unique entry only
					# with open('%s doc_dict_json.json'%str(state)) as f:
					#     doc_dict_json = json.load(f)
					sqlupdate = "SELECT `specs_script` FROM `hel_docinfo` WHERE `Doc_link`="+'"'+doc_link+'"'
					# print sqlupdate
					...
					# 	doc_dict_json[doc_link].append(str(type_doc)+'$$')
						...
						# doc_dict_json[doc_link].append(type_doc)
						res= cursor.fetchone ()
						res = res + ((str(spec)+'$$'),)
						
						res = MySQLdb.escape_string(str(res))
						# print res
						sqlupdate = "update `hel_docinfo` set `specs_script`="+'"'+str(res)+'"'+" where `Doc_link` ="+'"'+doc_link+'"'
						# print sqlupdate
					...
					# 	# print >>open ("terminal.txt",'a'),"above url not there"
					...
					# 	# print >>open ("terminal.txt",'a'),"above url type updated"

					# with open('Delhi doc_dict_json.json', 'w') as f:    #####dumping dictionary
					...
					if flag_pres ==1 :
						...
						sql1 = "INSERT INTO `hel_docinfo`( `Name`, `Recommemdation`, `Doc_link`, `Image-link`, `personal_link`, `Degree`,"
